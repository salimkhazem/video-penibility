...
def create_prediction_plot(
    ...
    ax1.set_title(
        f"Fold {fold} - Epoch {epoch} ({phase.title()})\nPredictions vs True Values"
    )
    ...
    def train_fold(
        ...
        with Progress(
            ...
            for epoch in range(self.training_config.num_epochs):
                # Training phase
                train_metrics, train_preds, train_targets, train_step = (
                    self._train_epoch(
                ...
                if writer is not None:
                    ...
                    for metric, value in train_metrics.items():
                        writer.add_scalar(
                            f"fold_{fold_num}/train/{metric}", value, epoch
                        )
                    ...
                    # Log learning rate
                    writer.add_scalar(
                        f"fold_{fold_num}/learning_rate",
                    ...
                    if epoch % 10 == 0 or epoch == self.training_config.num_epochs - 1:
                        ...
                        train_fig = create_prediction_plot(
                            train_targets, train_preds, fold_num, epoch, "train"
                        )
                        writer.add_figure(
                            f"fold_{fold_num}/train_predictions", train_fig, epoch
                        )
                        plt.close(train_fig)

                        # Validation predictions plot
                        val_fig = create_prediction_plot(
                            val_targets, val_preds, fold_num, epoch, "val"
                        )
                        writer.add_figure(
                        ...
                        writer.add_histogram(
                            f"fold_{fold_num}/val_predictions_hist", val_preds, epoch
                        )
                        writer.add_histogram(
                            f"fold_{fold_num}/train_targets_hist", train_targets, epoch
                        )
                        writer.add_histogram(
                            f"fold_{fold_num}/val_targets_hist", val_targets, epoch
                        )
                ...
                if patience_counter >= self.training_config.early_stopping_patience:
                    progress.update(
                        epoch_task, description=f"   Early stopped at epoch {epoch+1}"
                    )
                    break
    ...
    def _train_epoch(
        ...
        for batch_idx, (features, targets) in enumerate(train_loader):
            ...
            if writer is not None and batch_idx % 10 == 0:
                writer.add_scalar(
                    f"fold_{fold}/train_batch/loss", loss.item(), global_step
                )
                ...
                writer.add_scalar(
                    f"fold_{fold}/train_batch/grad_norm", total_norm, global_step
                )
