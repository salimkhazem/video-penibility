...
def print_data_info(config: MainConfig, dataset_size: int, feature_dim: int) -> None:
    ...
    table = Table(
        title="📊 Data Configuration", show_header=True, header_style="bold blue"
    )
    table.add_column("Parameter", style="cyan", no_wrap=True)
    table.add_column("Value", style="white")
...
def print_model_info(config: MainConfig, model: torch.nn.Module) -> None:
    ...
    table = Table(
        title="🧠 Model Architecture", show_header=True, header_style="bold green"
    )
    table.add_column("Parameter", style="cyan", no_wrap=True)
    table.add_column("Value", style="white")
...
def print_training_info(config: MainConfig) -> None:
    ...
    table = Table(
        title="⚙️ Training Configuration", show_header=True, header_style="bold yellow"
    )
    table.add_column("Parameter", style="cyan", no_wrap=True)
    table.add_column("Value", style="white")
    ...
    table.add_row(
        "Early Stopping Patience", str(config.training.early_stopping_patience)
    )
...
def create_prediction_plot(
    targets: np.ndarray, predictions: np.ndarray, fold: int, epoch: int
) -> plt.Figure:
...
def run_cross_validation(config: MainConfig, config_file: str) -> Dict[str, Any]:
    ...
    console.print(
        Panel.fit("🔄 Starting Cross-Validation Training", style="bold white on blue")
    )
    ...
    splits = create_subject_splits(
        dataset, config.cross_validation.n_splits, config.cross_validation.random_state
    )
    ...
    with Progress(
        ...
        cv_task = progress.add_task(
            "Cross-Validation Progress", total=config.cross_validation.n_splits
        )
        ...
        for fold, (train_indices, val_indices) in enumerate(splits):
            ...
            console.print(
                f"   Train samples: {len(train_indices)}, Validation samples: {len(val_indices)}"
            )
            ...
            trainer = TrainerFactory.create_trainer(
                config.training, feature_dim, config.model, config.experiment.device
            )
            ...
            # Brief fold summary
            console.print(
                f"   ✅ MSE: {fold_results['val_mse']:.3f}, R²: {fold_results['val_r2']:.3f}, CCC: {fold_results['val_ccc']:.3f}"
            )
...
def print_final_results(results: Dict[str, Any]) -> None:
    ...
    table = Table(
        title="🎯 Final Cross-Validation Results",
        show_header=True,
        header_style="bold magenta",
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train video penibility assessment model"
    )
    ...
    try:
        ...
        console.print(
            f"📊 [blue]View TensorBoard: tensorboard --logdir={Path(config.experiment.output_dir) / 'tensorboard'}[/blue]"
        )
